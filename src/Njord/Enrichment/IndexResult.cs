    ...
    (string Category, double Vpd)? Vpd)
{
    ...
    public IReadOnlyList<MqttMessage> ToMqttMessages(string baseTopic)
    {
        var payload = new JsonObject
        {
            ...
            ["hdd"] = JsonValue.Create(Math.Round(Hdd, 1)),
            ["cdd"] = JsonValue.Create(Math.Round(Cdd, 1)),
            ["solar"] = Solar,
            ["ventilation"] = Ventilation,
            ["frost_hours"] = FrostProtection?.HoursUntilFrost is { } fh ? JsonValue.Create(fh) : null,
            ["frost_confidence"] = FrostProtection?.Confidence is { } fc ? JsonValue.Create(fc) : null,
            ["vpd_category"] = Vpd?.Category,
            ["vpd_kpa"] = Vpd?.Vpd is { } v ? JsonValue.Create(v) : null,
        };

        var topic = TopicScheme.IndexTopic(baseTopic, Location);
