    ...
    (double DecayRate, int? ReliableHours)? Decay)
{
    ...
    public IReadOnlyList<MqttMessage> ToMqttMessages(string baseTopic)
    {
        ...
        foreach (var (apiName, trend) in ParameterTrends)
        {
        ...
        payload["weather_change"] = WeatherChange?.Description;

        payload["precip_starts"] = PrecipTiming.StartsInHours.HasValue
            ? JsonValue.Create(PrecipTiming.StartsInHours.Value) : null;
        payload["precip_ends"] = PrecipTiming.EndsInHours.HasValue
            ? JsonValue.Create(PrecipTiming.EndsInHours.Value) : null;

        payload["temp_max_in"] = ExtremaTiming.MaxInHours.HasValue
            ? JsonValue.Create(ExtremaTiming.MaxInHours.Value) : null;
        payload["temp_min_in"] = ExtremaTiming.MinInHours.HasValue
            ? JsonValue.Create(ExtremaTiming.MinInHours.Value) : null;

        payload["stability"] = Stability?.Label;
        payload["stability_ratio"] = Stability is { } s ? JsonValue.Create(s.Ratio) : null;

        payload["decay_rate"] = Decay is { } d ? JsonValue.Create(d.DecayRate) : null;
        payload["reliable_hours"] = Decay?.ReliableHours is { } rh ? JsonValue.Create(rh) : null;

        var topic = TopicScheme.TrendTopic(baseTopic, Location);
    ...
    {
        var targetTime = StatePayloadBuilder.Anchor(now, horizonHours);
