    ...
    int NightCooling)
{
    ...
    public IReadOnlyList<MqttMessage> ToMqttMessages(string baseTopic)
    {
        ...
        var payload = new JsonObject
        {
            ...
            ["cop_estimate"] = CopEstimate.HasValue ? JsonValue.Create(CopEstimate.Value) : null,
            ["cop_optimal"] = copArray,
            ["shading"] = Shading,
            ["battery_strategy"] = BatteryStrategy,
            ["night_cooling"] = NightCooling,
        };

        var topic = TopicScheme.EnergyTopic(baseTopic, Location);
