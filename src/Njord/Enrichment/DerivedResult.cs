    ...
    ScalarDerived Scalars)
{
    ...
    public IReadOnlyList<MqttMessage> ToMqttMessages(string baseTopic)
    {
        ...
        foreach (var (horizon, hd) in ByHorizon)
        {
            ...
            var topic = TopicScheme.DerivedHorizonTopic(baseTopic, Location, horizon);
            messages.Add(new MqttMessage(topic, payload.ToJsonString(), true));
        }

        var meta = new JsonObject
        {
            ["diurnal_amplitude"] = Scalars.DiurnalAmplitude.HasValue
                ? JsonValue.Create(Math.Round(Scalars.DiurnalAmplitude.Value, 1))
                : null,
            ["sunshine_pct"] = Scalars.SunshinePct.HasValue
                ? JsonValue.Create(Math.Round(Scalars.SunshinePct.Value, 1))
                : null,
            ["inversion"] = Scalars.Inversion.HasValue
                ? JsonValue.Create(Scalars.Inversion.Value)
                : null,
        };

        var metaTopic = TopicScheme.DerivedMetaTopic(baseTopic, Location);
