    ...
    double? WeightedTemperature)
{
    ...
    public IReadOnlyList<MqttMessage> ToMqttMessages(string baseTopic)
    {
        ...
        foreach (var (model, mae) in Mae7d)
            payload[$"mae_7d_{model.Id}"] = mae.HasValue ? JsonValue.Create(mae.Value) : null;

        foreach (var (model, mae) in Mae30d)
            payload[$"mae_30d_{model.Id}"] = mae.HasValue ? JsonValue.Create(mae.Value) : null;

        foreach (var (model, w) in Weights)
            payload[$"weight_{model.Id}"] = JsonValue.Create(w);

        foreach (var (model, d) in Drift)
            payload[$"drift_{model.Id}"] = d.HasValue ? JsonValue.Create(d.Value) : null;

        payload["seasonal_best"] = SeasonalBest?.Id;
        payload["anomaly"] = Anomaly?.IsAnomaly;
        payload["anomaly_deviation"] = Anomaly?.DeviationSigma is { } dev ? JsonValue.Create(dev) : null;
        payload["weighted_temperature"] = WeightedTemperature.HasValue ? JsonValue.Create(WeightedTemperature.Value) : null;

        var topic = TopicScheme.HistoryTopic(baseTopic, Location);
