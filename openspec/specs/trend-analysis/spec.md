...
`TrendAnalyzer.PredictabilityDecay` SHALL accept an `IReadOnlyList<(int HorizonHours, double? Spread)>` of consensus spreads at each horizon. It SHALL compute a linear regression slope of spread vs horizon hours, ignoring null spreads. It SHALL return `(double DecayRate, int? ReliableHours)?`. `DecayRate` is the slope (spread increase per hour). `ReliableHours` is the first horizon where spread exceeds a threshold (default 3.0 degrees C), or `null` if it never does. If fewer than 2 non-null data points exist, the result SHALL be `null`.
